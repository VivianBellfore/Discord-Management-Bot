...
namespace LCNET_Management_Bot
{
    ...
    /// </summary>
    internal class SetTempVoiceCategory : CommandObject
    {
        /// <summary>
        /// Strukt for the help command informations.
        /// </summary>
        internal SetTempVoiceCategory() : base("guild", "voice", "command_guild_voice") { }
        ...
        internal async override Task CommandFunction(SocketSlashCommand command)
        {
            ...
            int updateCount = await MySqlWrapper.SQLExecuteNonQuery(
                    "UPDATE `guild_channel` SET `tempvoice` = @category_voice WHERE `guild_id` = @guild_id",
                    new Dictionary<string, object>() { { "guild_id", (ulong)command.GuildId }, { "category_voice", channel.Id } });

            if (updateCount > 0)
            {
                int insertCount = await MySqlWrapper.SQLExecuteNonQuery(
                    $"INSERT INTO `guild_channel` (`guild_id`, `tempvoice`) VALUES (@guild_id, @category_ticket)",
